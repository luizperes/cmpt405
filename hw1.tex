...
\textbf{\#3}\\
The idea here is to use the greedy approach of the \textbf{set cover} problem. Let $U$ be the the collection with all tiles $a_{ij}$.\\\\
Algorithm:\\\\
\textbf{Input:} \textit{U}
\begin{algorithmic}
\For{$i$ \textbf{from} $1$ \textbf{to} $K$}
  ...
  \EndFor
\EndFor
\State $S \gets$ create sets $S_1, S_2,...,S_{max(n_i)}$ using adjacent tiles in $arr$. 
\State $C \gets 0$
