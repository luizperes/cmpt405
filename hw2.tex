...
\textbf{\#2}
\\
\textit{Definition:} Let $A$ be an array with size $n+1$. Initialize $A[0] = 1$, $A[1]=0$. Define $A[i]$ as the number of possible orders in which you can multiply after an iteration $i$.  At the end, the number total of possible orders will be $A[n]$.
\\
\\
\textit{Recurrence:}
\\
